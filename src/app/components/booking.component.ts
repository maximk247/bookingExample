...
import { BookingData, BookingService, RoomType } from './booking.service';
  ...
  private initForm() {
    this.bookingForm = new FormGroup({
      firstName: new FormControl('', [
        ...
        Validators.pattern('[a-zA-Zа-яА-ЯёЁ]*'), // Допускаются только буквы
      ]),
      lastName: new FormControl('', [
        Validators.required, // Обязательное поле
        Validators.pattern('[a-zA-Zа-яА-ЯёЁ]*'), // Допускаются только буквы
      ]),
      patronymicName: new FormControl('', Validators.pattern('[a-zA-Zа-яА-ЯёЁ]*')), // Допускаются только буквы
    ...
    console.log(this.bookingForm.get('roomTypeId')?.value);
    this.handleRoomTypeChanges(); // Обрабатываем изменения в типе номера
  }

  private handleRoomTypeChanges() {
    this.bookingForm.get('roomTypeId')?.valueChanges.subscribe((roomTypeId) => {
      let maxGuests = 10;
      let signal = false
      switch (roomTypeId) {
        case '0':
